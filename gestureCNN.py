...
# WeightFileName = ["ori_4015imgs_weights.hdf5"]
...
def loadCNN(wf_index):
    global get_output
    model = Sequential()
    model.add(Conv2D(nb_filters, (nb_conv, nb_conv),padding='valid',input_shape=(img_rows, img_cols, img_channels)))
...
def initializers():
    ...
    immatrix = np.array([
        np.array(Image.open(path2+ '/' + images).convert('L')).flatten()for images in imlist], dtype = 'f')
...
def visualizeHis(hist):
    ...
    # Tloss,Vloss,Tacc,Vacc = history.history['loss'],history.history['val_loss'],history.history['acc'],history.history['val_acc']
    # xc = range(epoch)
    # plt.figure(1,figsize=(7,5))
    # plt.plot(xc,Tloss)
    # plt.plot(xc,Vloss)
    # plt.xlabel('Epochs')
    # plt.ylabel('Train Loss VS Val')
